...
install_shadowsocks() {
  check_and_install_xray
  XRAY_BIN=$(command -v xray || echo "/usr/local/bin/xray")
  read -rp "请输入 Shadowsocks 监听端口（如 8388）: " PORT
  read -rp "请输入加密方式（推荐 aes-128-gcm，默认aes-128-gcm）: " METHOD
  METHOD=${METHOD:-aes-128-gcm}
  read -rp "请输入密码（不填自动生成）: " PASSWORD
  if [ -z "$PASSWORD" ]; then
    PASSWORD=$(openssl rand -hex 8)
  fi
  read -rp "节点备注（备注名）: " REMARK
...
EOF
  ...
  IP=$(curl -s ipv4.ip.sb || curl -s ifconfig.me)
  # SS链接格式：ss://base64( method:password )@IP:PORT#remark
  userpass=$(echo -n "$METHOD:$PASSWORD" | base64 -w 0)
  LINK="ss://$userpass@$IP:$PORT#$REMARK"
  green "✅ Shadowsocks 节点链接："
  echo "$LINK"
  read -rp "按任意键返回菜单..."
}
...
install_socks5() {
  check_and_install_xray
  XRAY_BIN=$(command -v xray || echo "/usr/local/bin/xray")
  read -rp "请输入 Socks5 监听端口（如 1080）: " PORT
  read -rp "是否开启用户名密码认证？(y/n): " AUTH
...
while true; do
  ...
  echo "10) 安装 Shadowsocks (SS) 节点"
  echo "11) 安装 Socks5 节点"
  echo "0) 退出"
  echo
